    ...
    <div class="form">
      <div v-for="(it,index) in formData" class="item">
        ...
        <group class="describe">
          ...
          <x-input title="工时数量(小时)"
                   placeholder="请输入最多一位"
                   text-align="right"
                   :show-clear="false"
                   v-model="it.time"
                   @on-blur="timeValid(it.time)">
          </x-input>
        </group>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    submit() {
      this.postAjax(this.$api.workingInfo, { working_info: this.formData, order_id: this.repairInfo.baseInfo.mtc_a10_order_id })
        .then(res => {
    ...
    fillData() {
      if (this.repairInfo.workTimeItem.length) {
        this.formData = this.repairInfo.workTimeItem
      }
    },
    timeValid(value) {
      this.blur();
      if (value && ! /^[0-9]+(.[0-9]{1})?$/.test(value)) {
        this.$vux.toast.show('工时数量只可以输入数字或带一位小数')
      }
    }
  }
}

<template>
  <div class="m-cont">
    <Head :headData="1">零配件</Head>    <div class="form">
      <div v-for="(it,index) in formData" class="item">
        <div class="header">
          <div>
            <span class="workTime">配件信息(<span>{{ index+1 }}</span>)</span>
...
export default {
  ...
  methods: {
    ...
    submit() {
      this.postAjax(this.$api.installationKit, { parts_info: this.formData, order_id: this.repairInfo.baseInfo.mtc_a10_order_id })
        .then(res => {
    ...
    fillData() {
      if (this.repairInfo.partsInfo.length) {
        this.formData = this.repairInfo.partsInfo;
      }
    },
  }
}
